...
def ask_ai(chat_id, user_question):
    ...
    # если ключа нет, молча выходим
    if client is None:
        log('ask_ai: client is None — GEMINI_API_KEY не задан', 'Ошибка')
        return None
...
def send_reply_keyboard(chat_id):
    # нижнее постоянное меню — три кнопки
    url = f'{API}/sendMessage'
    payload = {
        'chat_id': chat_id,
        ...
        'reply_markup': json.dumps({
            'keyboard': [
                [{'text': '▶️ Старт'}, {'text': '📊 Статистика'}, {'text': '❓ Помощь'}],
            ],
            'resize_keyboard': True
        }, ensure_ascii=False)
    }
...
def kb_main(user=None):
    # минималистичная клавиатура с цветами
    return {
        'inline_keyboard': [
            [
                {'text': '▶️ Старт', 'callback_data': 'work', 'style': 'primary'},
                {'text': '📊 Статистика', 'callback_data': 'stats', 'style': 'success'},
                {'text': '❓ Помощь', 'callback_data': 'help', 'style': 'danger'}
            ]
        ]
    }
    ...
    def fire():
        ...
        try:
            ...
            if mode == 'work':
                ...
                # спрашиваем ии совет для перерыва
                ai_tip = ask_ai(chat_id, 'Дай один короткий совет для перерыва после 25 минут работы. Одно предложение.')
                ...
                if ai_tip:
                    text += f'\n\n💡 <b>Совет от ИИ:</b> {ai_tip}'
                text += '\n\n<i>Можешь спросить у меня что угодно — просто напиши сообщением.</i>'

                send_message(chat_id, text, kb_main(user))
            elif mode == 'short_break':
                ai_tip = ask_ai(chat_id, 'Дай короткий совет, как вернуться к работе после перерыва. Одно предложение.')
                text = '⏰ <b>Перерыв закончился!</b>\n\n'
                if ai_tip:
                    text += f'💡 <b>Совет от ИИ:</b> {ai_tip}\n\n'
                text += 'Готов к следующей помидорке?'
                send_message(chat_id, text, kb_main(user))
            elif mode == 'long_break':
                ai_tip = ask_ai(chat_id, 'Дай короткий совет, как продуктивно начать новый цикл после длинного перерыва. Одно предложение.')
                text = '⏰ <b>Длинный перерыв закончился!</b>\n\n'
                if ai_tip:
                    text += f'💡 <b>Совет от ИИ:</b> {ai_tip}\n\n'
            ...
            if message:
                ...
                if user_text == '/start':
                    ...
                    send_message(
                        ...
                        f'Я - Pomodoro-бот. Помогу работать по методу «помидора»: '
                        f'25 минут фокуса, 5 минут отдыха.\n\n'
                        f'🤖 <b>У меня есть ИИ-помощник</b> — можешь задать мне любой вопрос '
                        f'о продуктивности, концентрации или тайм-менеджменте, '
                        f'просто написав сообщение.\n\n'
                        f'Или выбери действие в меню ниже 👇',
                        kb_main(db_get_user(chat_id))
                    )
                elif user_text == '/menu':
                    handle_menu(chat_id)
